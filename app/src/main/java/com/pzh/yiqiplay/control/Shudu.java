...
public class Shudu {
    ...
    // 输出数组
    public static ShuduBean[][] initArray(int[][] array) {
        ShuduBean[][] shuduArray = new ShuduBean[array.length][array[1].length];
        for (int i = 0; i < array.length; i++) {
            for (int j = 0; j < array[i].length; j++) {
                ShuduBean bean=new ShuduBean();
                bean.setCheckable(false);
                bean.setNum(array[i][j]);
                shuduArray[i][j]=bean;
            }
    ...
    public static ShuduBean[][] initShuduArray() {
        Shudu re = new Shudu();
